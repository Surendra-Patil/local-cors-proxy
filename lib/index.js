    ...
    req.on('end', () => {
      ...
      const makeRequest = () => {
        ...
        reqStream.on('response', response => {
          ...
          // Track response size
          let responseSize = 0;
          const responseSizeLimit = 100 * 1024 * 1024; // 100MB
          
          // Create a transform stream to track response size
          const responseTracker = new require('stream').Transform({
            transform(chunk, encoding, callback) {
              responseSize += chunk.length;
              if (responseSize > responseSizeLimit) {
                console.log(chalk.yellow(`Large response detected: ${responseSize} bytes`));
              }
              this.push(chunk);
              callback();
            }
          ...
          // On response completion, log total size
          response.on('end', () => {
        ...
        reqStream.pipe(responseTracker).pipe(res);
      };
